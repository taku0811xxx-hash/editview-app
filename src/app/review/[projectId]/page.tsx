...
import { getComments, addComment, markCommentsAsSent, deleteComment, formatTimecode } from '@/lib/comments'
...
export default function ReviewPage() {
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
      <header className="bg-gray-950 border-b border-gray-800 h-12 flex items-center px-6 justify-between shrink-0">
        <div className="flex items-center gap-4">
          <svg width="100" height="28" viewBox="0 0 110 32" fill="none" aria-label="Editview">
            ...
            <text x="66" y="22" fontFamily="Georgia,serif" fontSize="18" fontWeight="400" fill="#777" letterSpacing="-0.5">view</text>
          </svg>
          <div className="w-px h-4 bg-gray-700" />
          <div>
            <p className="text-sm font-medium text-white">{project?.title}</p>
            <p className="text-xs text-gray-500">Ver.{currentVersion?.versionNumber ?? '-'} · {project?.clientName}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-500">修正 {project?.revisionCount ?? 0}/{project?.revisionLimit ?? '-'}回</span>
          <span className="text-xs bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-full font-medium">確認中</span>
        </div>
      </header>

      {!nameSet && (
        <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3 shrink-0">
          <p className="text-xs text-gray-700 font-medium">お名前を入力してください（コメントに表示されます）</p>
          <form onSubmit={handleSetName} className="flex gap-2">
            <input value={authorName} onChange={e => setAuthorName(e.target.value)} placeholder="例: 田中" className="border border-gray-300 bg-white text-gray-900 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-gray-400 placeholder:text-gray-400" required />
            <button type="submit" className="text-xs bg-gray-900 text-white px-3 py-1.5 rounded-lg hover:bg-gray-700 font-medium transition-colors">設定</button>
          </form>
        </div>
      )}

      <div className="flex flex-1 min-h-0">
        <div className="flex-1 border-r border-gray-100 flex flex-col overflow-y-auto">
          <div className="bg-gray-100 p-3 shrink-0">
        ...
        <div className="bg-white flex flex-col overflow-hidden" style={{ width: '35%' }}>
          <div className="px-4 py-3 border-b border-gray-100 shrink-0">
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-gray-400">現在位置: <span className="font-mono text-gray-700 font-medium">{formatTimecode(currentTimecode)}</span></p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4" style={{ minHeight: 0 }}>
              ...
              return (
                <div key={round}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-semibold text-gray-400 bg-gray-100 px-2.5 py-1 rounded-full">第{round}回修正（送信済み）</span>
                  ...
                  <div className="space-y-2 opacity-50">
                    {roundComments.map(comment => {
                      ...
                      return (
                        <div key={comment.id}>
                          <div className="border border-gray-100 bg-gray-50/80 rounded-xl p-4">
            ...
            {unsentComments.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-semibold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full">第{nextRevisionCount}回修正（未送信）</span>
                      ...
                      <div key={comment.id}>
                        <div className="border border-purple-100 bg-purple-50/40 rounded-xl p-4">
                          <div className="flex items-center gap-1.5 mb-2">
                            <button
                              onClick={() => { if (videoRef.current) videoRef.current.currentTime = comment.timecode }}
                              className="font-mono text-xs font-bold bg-purple-100 text-purple-700 rounded-lg px-2.5 py-1 hover:bg-purple-200"
                            >
                              {formatTimecode(comment.timecode)}
                            </button>
                            <span className="text-xs text-gray-400 flex-1">{comment.authorName}</span>
                            <button
                              onClick={() => handleDeleteComment(comment.id)}
                              className="text-xs text-gray-300 hover:text-red-500 transition-colors px-1.5 py-0.5 rounded hover:bg-red-50"
                              title="削除"
                            >
                              ✕
                            </button>
                        ...
                        {replies.length > 0 && (
                          <div className="ml-3 mt-1 space-y-1 border-l-2 border-blue-100 pl-2">
                            {replies.map(reply => (
                              <div key={reply.id} className="border border-blue-100 bg-blue-50/40 rounded-xl p-3">
          ...
          <div className="p-4 border-t border-gray-100 space-y-2 shrink-0">
            <form onSubmit={handleAddComment} className="space-y-2">
              <textarea
                ...
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm resize-none focus:outline-none focus:border-gray-400"
                rows={3}
                disabled={!nameSet}
              />
              <button
                ...
                className="w-full border border-gray-200 rounded-xl py-2.5 text-xs font-medium hover:bg-gray-50 disabled:opacity-40 transition-colors"
              >
            ...
            <div className="border-t border-gray-100 pt-2 space-y-2">
              ...
              <button
                ...
                disabled={approving}
                className="w-full border border-green-200 rounded-xl py-3 text-sm font-medium text-green-700 bg-green-50 hover:bg-green-100 disabled:opacity-50 transition-colors"
              >
